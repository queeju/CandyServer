...
func BuyCandyOperation(params BuyCandyParams) middleware.Responder {
	order := params.Order

	if *order.CandyCount <= 0 {
		return NewBuyCandyBadRequest().WithPayload(&BuyCandyBadRequestBody{
			Error: "Invalid candy count",
		})
	}
	...
	totalCost := price * int(*order.CandyCount)
	if int(*order.Money) < totalCost {
		errorMsg := fmt.Sprintf("You need %d more money!", totalCost-int(*order.Money))
	...
	cResponse := C.ask_cow(phrase)
	thanks := C.GoString(cResponse)
	response := &BuyCandyCreatedBody{
		Thanks: thanks,
		Change: int64(change),
	}
	return NewBuyCandyCreated().WithPayload(response)
}
...
func (o *BuyCandyBody) Validate(formats strfmt.Registry) error {
	...
	if err := o.validateCandyCount(); err != nil {
		res = append(res, err)
	}

	if err := o.validateCandyType(); err != nil {
		res = append(res, err)
	}

	if err := o.validateMoney(); err != nil {
		res = append(res, err)
	}
...
func (o *BuyCandyBody) validateCandyCount() error {
...
func (o *BuyCandyBody) validateCandyType() error {
...
func (o *BuyCandyBody) validateMoney() error {
